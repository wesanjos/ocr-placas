...
const detectLicensePlate = (src, dst) => {
  ...
  try {
    ...
    cv.drawContours(contoursImage, contours, -1, new cv.Scalar(0, 255, 0, 255), 2);
    ...
    for (let i = 0; i < contours.size(); ++i) {
      ...
      const aspectScore = Math.max(0, 1 - Math.abs(aspectRatio - PLATE_ASPECT_RATIO) / 2);
      ...
      if (score > maxScore && score > 0.5) { // Threshold de pontuação
        maxScore = score;
    ...
    if (plateContour) {
      ...
      const srcPoints = cv.matFromArray(4, 1, cv.CV_32FC2, [].concat(...srcPointsArray));
      const dstPointsMat = cv.matFromArray(4, 1, cv.CV_32FC2, [].concat(...dstPointsArray));
      
      // Calcular matriz de perspectiva
      const perspectiveMatrix = cv.getPerspectiveTransform(srcPoints, dstPointsMat);
  ...
  try {
    ...
    if (plateResult.found) {
      ...
      if (!isProcessingFrame) {
        ...
        // Notificar o usuário
        resultElement.textContent = "Placa detectada! Processando imagem capturada...";
...
const processPlateFrame = async (plateResult) => {
  try {
    ...
    cv.threshold(plateSharpened, plateThresh, 0, 255, cv.THRESH_BINARY + cv.THRESH_OTSU);
    ...
    cv.resize(plateThresh, plateResized, new cv.Size(newWidth, newHeight), 0, 0, cv.INTER_CUBIC);
    ...
    cv.morphologyEx(plateResized, plateProcessed, cv.MORPH_CLOSE, kernel, new cv.Point(-1, -1), 1);
    ...
    if (processedText.length >= 7) {
      ...
      const isNewFormat = /[A-Z]{3}[0-9][A-Z][0-9]{2}/.test(processedText.substring(0, 7));
      const isOldFormat = /[A-Z]{3}[0-9]{4}/.test(processedText.substring(0, 7));
    ...
    resultElement.innerHTML = `<strong>Texto reconhecido:</strong> ${processedText || "Nenhum texto reconhecido na placa."}` + 
    ...
    resultElement.textContent = "Erro no processamento da placa: " + error.message;
